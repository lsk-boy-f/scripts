...
This script extracts and analyzes SO files from APK packages and summarizes by alignment.

Usage: check_so_load.py [input-path|input-APK]
...
def get_llvm_objdump_path():
    ...
    llvm_objdump = os.path.join(android_ndk, "toolchains", "llvm", "prebuilt", "darwin-x86_64", "bin", "llvm-objdump")
    
    if not os.path.isfile(llvm_objdump):
        print(f"Error: llvm-objdump not found at {llvm_objdump}", file=sys.stderr)
    ...
    try:
        with zipfile.ZipFile(apk_path, 'r') as zip_ref:
            ...
            if lib_files:
                ...
                print("Warning: No lib/*.so files found, extracting all files...")
...
Environment variables:
  ANDROID_NDK - Path to Android NDK (required)
    ...
    parser.add_argument('input_path', help='Path to SO files or APK file')
    ...
    try:
        ...
        if input_path.endswith('.apk'):
            ...
            temp_dir = tempfile.mkdtemp(prefix=f"{Path(input_path).stem}_so_load_")
        ...
        if alignment_count:
            ...
            for align in sorted(alignment_count.keys(), key=lambda x: int(re.search(r'2\*\*(\d+)', x).group(1)) if re.search(r'2\*\*(\d+)', x) else 0):
